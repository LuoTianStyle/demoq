...
axios.interceptors.request.use(function (data) {
  if (
    ...
    data.headers.common['token'] = JSON.parse(
      localStorage.getItem('userData')
    ).token
  }
  return data
...
export const fileDelShare = params => {
  return axios.post(`${path}/file/delShare`, params).then(res => res.data)
}
